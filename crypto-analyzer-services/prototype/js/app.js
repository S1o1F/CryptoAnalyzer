...
async function loadData() {
    try {
        ...
        try {
            ...
            console.error('Make sure the API server is running on http://localhost:8000');
            console.error('You can start it by running: python main.py (from the dians_hw3 directory)');
...
async function showCryptoInfo(symbol) {
    ...
    try {
        // Fetch technical analysis with proper error handling
        try {
            ...
            if (analysisResponse.ok) {
                ...
                console.warn(`[ANALYSIS] Failed to load analysis: ${analysisResponse.status} ${analysisResponse.statusText}`);
            }
        } catch (fetchError) {
            console.error('[ANALYSIS] Fetch error:', fetchError);
            console.error('Make sure the API server is running on http://localhost:8000');
            console.error('You can start it by running: python main.py');
        }

        // Fetch price history with proper error handling
        try {
            ...
            if (historyResponse.ok) {
                ...
                if (history.length > 0) {
                    ...
                    document.getElementById('cryptoInfoContent').innerHTML = 
                        '<p style="text-align:center;padding:2rem;color:var(--danger);">No historical data available for this cryptocurrency.</p>';
                }
            } else {
                console.warn(`[HISTORY] Failed to load history: ${historyResponse.status} ${historyResponse.statusText}`);
                document.getElementById('cryptoInfoContent').innerHTML = 
                    '<p style="text-align:center;padding:2rem;color:var(--danger);">Failed to load price history. Please try again later.</p>';
            }
        } catch (fetchError) {
            console.error('[HISTORY] Fetch error:', fetchError);
            document.getElementById('cryptoInfoContent').innerHTML = 
                '<p style="text-align:center;padding:2rem;color:var(--danger);">Cannot connect to API server. Please make sure the server is running on http://localhost:8000<br><br>Start the server with: <code>python main.py</code></p>';
        }

        // Load on-chain metrics with error handling
        try {
            await loadOnChainMetrics(symbol);
        } catch (metricsError) {
            console.error('[ONCHAIN] Error loading metrics:', metricsError);
        }
    } catch (error) {
        console.error('[ERROR] Unexpected error in showCryptoInfo:', error);
        document.getElementById('infoCurrentPrice').textContent = 'Error loading data';
        document.getElementById('cryptoInfoContent').innerHTML = 
            '<p style="text-align:center;padding:2rem;color:var(--danger);">An unexpected error occurred. Please check the console for details.</p>';
    }
}
...
async function loadOnChainMetrics(symbol) {
    try {
        ...
        if (!response.ok) {
            console.warn(`[ONCHAIN] Could not load metrics for ${symbol}: ${response.status} ${response.statusText}`);
            if (document.getElementById('onchainMetricsCard')) {
